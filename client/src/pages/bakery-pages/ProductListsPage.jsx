...
function ProductListsPage() {
    ...
    const fetchProducts = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_HOST}/api/products/bakery`, {
                headers: {
    ...
    const handleUpdateStock = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_HOST}/api/products/${selectedProductId}/stock`, {
                method: 'PATCH',
                headers: {
    ...
    const handleDeleteProduct = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND_HOST}/api/products/${selectedProductId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map((product) => (
                    <div key={product._id} className="bg-white p-6 rounded-lg shadow-md">
                        <img
                            src={`${process.env.REACT_APP_BACKEND_HOST}/uploads/${product.image}` || '/fallback-image.jpg'}
                            alt={product.name}
